...
document.addEventListener('DOMContentLoaded', async () => {
    mermaid.initialize({
        ...
        theme: 'dark',
        themeVariables: {
            clusterBkg: 'transparent' // Bỏ nền xám của subgraph
        },
        maxTextSize: 9000000,
        securityLevel: 'loose',
...
async function loadData() {
    try {
        ...
        // 1. Tải master.md
        const masterUrl = '../ai/master.md';
        const masterText = await fetch(masterUrl, freshData).then(r => r.text());
        
        // 2. Tìm tất cả các link file con trong master.md (vd: [Dashboard](dashboard.md))
        const linkRegex = /\]\((.*?\.md)\)/g;
        const fileUrls = [];
        let match;
        while ((match = linkRegex.exec(masterText)) !== null) {
            fileUrls.push('../ai/' + match[1]);
        }
        ...
        let loadedAudit = {};
        let loadedSecurity = {};
        let loadedSourceIntel = {};
        try {
            const jsonResponses = await Promise.all([
                fetch('data/manifest.json', freshData).then(r => r.ok ? r.json() : {}),
                fetch('data/dashboard.json', freshData).then(r => r.ok ? r.json() : {}),
                fetch('data/audit-details.json', freshData).then(r => r.ok ? r.json() : {}),
                fetch('data/security-scans.json', freshData).then(r => r.ok ? r.json() : {}),
                fetch('data/source_intelligence.json', freshData).then(r => r.ok ? r.json() : {})
            ]);
            loadedManifest = jsonResponses[0];
            loadedDashboard = jsonResponses[1];
            loadedAudit = jsonResponses[2];
            loadedSecurity = jsonResponses[3];
            loadedSourceIntel = jsonResponses[4];
        } catch(e) {
            console.error("Failed to load JSON data", e);
        }

        // 5. Gộp tất cả thành 1 file MD khổng lồ ảo trong RAM để parse như cũ
        const mdText = masterText + '\n\n' + contents.join('\n\n');
        
        manifestData = { bugs: [], aiPlans: loadedManifest.aiPlans || [] };
        workflowsData = {};
        dashboardData = { 
            corePrinciples: [], 
            scalingRoadmap: [], 
            changelog: [], 
            moduleBugs: [],
            auditReport: loadedDashboard.auditReport || null,
            dependencies: loadedDashboard.dependencies || null
        };
        auditDetailsData = loadedAudit;
        securityScansData = loadedSecurity;
        window.sourceIntelligenceData = loadedSourceIntel;
        ...
        for (let token of tokens) {
            if (token.type === 'heading' && token.depth === 1) {
                ...
                if (title.includes('dashboard')) currentSection = 'dashboard';
                else if (title.includes('workflows')) currentSection = 'workflows';
                else if (title.includes('bugs')) currentSection = 'bugs';
            ...
            // --- WORKFLOWS ---
                ...
                if (token.type === 'heading' && token.depth === 2) {
                    currentWfId = token.text.toLowerCase().replace(/[^a-z0-9]+/g, '-');
                    workflowsData[currentWfId] = { title: token.text, icon: '⚙️', mermaid: '' };
                }
                else if (currentWfId && token.type === 'list') {
                    token.items.forEach(li => {
                        if (li.text.startsWith('**Title:**')) workflowsData[currentWfId].title = li.text.replace('**Title:**', '').trim();
                        if (li.text.startsWith('**Icon:**')) workflowsData[currentWfId].icon = li.text.replace('**Icon:**', '').trim();
                    });
                }
                else if (currentWfId && token.type === 'code' && token.lang === 'mermaid') {
                    workflowsData[currentWfId].mermaid = token.text;
                }
            }
            ...
            // --- BUGS ---
                ...
                if (token.type === 'heading' && token.depth === 2) {
                    const match = token.text.match(/^([A-Z0-9_-]+):\s*(.+)$/);
                    if (match) {
                        currentBug = { id: match[1], title: match[2], status: 'open', severity: 'low', module: '', affectedFiles: [] };
                        manifestData.bugs.push(currentBug);
                    }
                }
                else if (currentBug && token.type === 'list') {
                    token.items.forEach(li => {
                        const txt = li.text;
                        if (txt.startsWith('**Status:**')) currentBug.status = txt.replace('**Status:**', '').trim();
                        if (txt.startsWith('**Severity:**')) currentBug.severity = txt.replace('**Severity:**', '').trim();
                        if (txt.startsWith('**Module:**')) currentBug.module = txt.replace('**Module:**', '').trim();
                        if (txt.startsWith('**Files:**')) currentBug.affectedFiles = txt.replace('**Files:**', '').split(',').map(s=>s.trim());
                    });
                }
                else if (currentBug && token.type === 'heading' && token.depth === 3) {
                    currentItem = token.text.toLowerCase();
                }
                else if (currentBug && currentItem && token.type === 'paragraph') {
                    if (currentItem === 'symptom') currentBug.symptom = token.text;
                    if (currentItem === 'cause') currentBug.cause = token.text;
                    if (currentItem === 'solution') currentBug.solution = token.text;
                }
                else if (currentBug && currentItem === 'code' && token.type === 'code') {
                    currentBug.code = token.text;
                }
            }
            ...
            // --- DASHBOARD ---
                ...
                else if (currentItem === 'core principles' && token.type === 'heading' && token.depth === 3) {
                    dashboardData.corePrinciples.push({ topic: token.text, rule: '' });
                }
                else if (currentItem === 'core principles' && token.type === 'paragraph') {
                    const last = dashboardData.corePrinciples[dashboardData.corePrinciples.length - 1];
                    if (last) last.rule += token.text + '\n';
                }
                else if (currentItem === 'scaling roadmap') {
                    if (token.type === 'heading' && token.depth === 3) {
                        dashboardData.scalingRoadmap.push({ name: token.text, status: '', color: 'blue', desc: '', tasks: [] });
                    }
                    else if (token.type === 'list') {
                        const last = dashboardData.scalingRoadmap[dashboardData.scalingRoadmap.length - 1];
                        if (last) {
                            token.items.forEach(li => {
                                const txt = li.text;
                                if (txt.startsWith('**Status:**')) last.status = txt.replace('**Status:**', '').trim();
                                else if (txt.startsWith('**Color:**')) last.color = txt.replace('**Color:**', '').trim();
                                else last.tasks.push(txt.replace(/^\s*\[[ x]\]\s*/, ''));
                            });
                        }
                    }
                    else if (token.type === 'paragraph') {
                        const last = dashboardData.scalingRoadmap[dashboardData.scalingRoadmap.length - 1];
                        if (last) last.desc += token.text + '\n';
                    }
                }
                else if (currentItem === 'changelog') {
                    if (token.type === 'heading' && token.depth === 3) {
                        dashboardData.changelog.push({ date: token.text, color: 'blue', summary: '', details: [] });
                    }
                    else if (token.type === 'list') {
                        const last = dashboardData.changelog[dashboardData.changelog.length - 1];
                        if (last) {
                            token.items.forEach(li => {
                                const txt = li.text;
                                if (txt.startsWith('**Color:**')) last.color = txt.replace('**Color:**', '').trim();
                                else if (txt.startsWith('**Summary:**')) last.summary = txt.replace('**Summary:**', '').trim();
                                else last.details.push(txt);
                            });
                        }
                    }
                }
        ...
        const mods = {};
        manifestData.bugs.forEach(b => {
            if (!mods[b.module]) mods[b.module] = { module: b.module, color: 'accent-color', bugs: [] };
            mods[b.module].bugs.push({ id: b.id, title: b.title, status: b.status });
        });
        dashboardData.moduleBugs = Object.values(mods);

    } catch (e) {
        console.error("Failed to load ROADMAP.md:", e);
        document.getElementById('dynamic-content').innerHTML = `<h3 style="color:red">Lỗi tải ROADMAP.md</h3><p>${e.message}</p>`;
    }
}
...
function renderSidebar() {
    ...
    html += `
        ...
        <div class="menu-section">
            ...
            <a class="menu-item" id="nav-source-intel" onclick="navigateTo('source-intel')">🧠 Source Intelligence</a>
...
async function navigateTo(view, param = null) {
    ...
    if (view === 'dashboard') {
        ...
        renderRoadmapView();
    } else if (view === 'source-intel') {
        document.getElementById('nav-source-intel').classList.add('active');
        viewTitle.innerHTML = '🧠 Source Intelligence';
        renderSourceIntelView();
...
function renderAuditView() {
    ...
    if (auditDetailsData) {
        ...
        // Helper function to render a table from JSON
            ...
            rows.forEach(row => {
                ...
                row.forEach(cell => {
                    if (typeof cell === 'object' && cell.text) {
                        tHtml += `<td class="${cell.class || ''}">${cell.text}</td>`;
                    } else {
                        tHtml += `<td>${cell}</td>`;
                    }
                });
        ...
        if (auditDetailsData.sections) {
            auditDetailsData.sections.forEach(sec => {
                ...
                if (sec.type === 'table') {
                    html += renderTable(sec.headers, sec.rows);
                } else if (sec.type === 'cost') {
                    html += `<div style="margin: 1.5rem 0;">`;
                    sec.bars.forEach(bar => {
                        const barColor = bar.color ? `background: var(--${bar.color});` : '';
                        html += `
                            <div class="cost-bar">
                                <div class="val" style="min-width: 150px;">${bar.label}</div>
                                <div class="bar" style="width: ${bar.width}; ${barColor}"></div>
                                <div class="val">${bar.value}</div>
                            </div>
                        `;
                    });
                    html += `</div><div style="display: flex; gap: 1rem; margin-top: 1.5rem; flex-wrap: wrap;">`;
                    sec.cards.forEach(card => {
                        html += `
                            <div class="card" style="padding: 1rem 2rem; text-align: center; flex: 1; min-width: 200px;">
                                <div style="font-size: 2rem; font-weight: 800; color: var(--${card.color});">${card.value}</div>
                                <div style="font-size: 0.8rem; color: var(--text-muted);">${card.label}</div>
                            </div>
                        `;
                    });
                    html += `</div>`;
                    if (sec.note) {
                        html += `
                            <div style="margin-top: 1rem; padding: 0.75rem; background: rgba(88, 166, 255, 0.1); border-radius: 8px; font-size: 0.85rem; color: var(--${sec.noteColor || 'primary'});">
                                ${sec.note}
                            </div>
                        `;
                    }
                } else if (sec.type === 'mixed') {
                    sec.blocks.forEach(block => {
                        if (block.type === 'diagram') {
                            html += `<div class="flow-diagram" style="${block.align ? 'text-align: ' + block.align + '; padding: 1.5rem;' : ''}">${block.content}</div>`;
                        } else if (block.type === 'heading') {
                            html += `<h4 style="margin: 1.5rem 0 0.75rem; color: var(--text);">${block.content}</h4>`;
                        } else if (block.type === 'table') {
                            html += renderTable(block.headers, block.rows);
                        } else if (block.type === 'code') {
                            html += `<div class="schema-block">${block.content.replace(/</g, '&lt;').replace(/>/g, '&gt;')}</div>`;
                        } else if (block.type === 'alert') {
                            html += `
                                <div style="margin-top: 1rem; padding: 0.75rem; background: rgba(63, 185, 80, 0.1); border-radius: 8px; font-size: 0.85rem; color: var(--${block.status});">
                                    ${block.content}
                                </div>
                            `;
                        }
...
window.loadSecurityScanFile = async function (fileId) {
    ...
    try {
        ...
        reader.innerHTML = `
            ...
            <p class="card-desc">Hãy mở roadmap bằng static server từ thư mục gốc dự án để truy cập được <code>codex-security-scans</code>.</p>
        `;
    }
};
...
// Render Source Intelligence View
function renderSourceIntelView() {
    const content = document.getElementById('dynamic-content');
    const data = window.sourceIntelligenceData;
    
    if (!data || !data._meta) {
        content.innerHTML = '<div class="dashboard-view"><p>Chưa có dữ liệu Source Intelligence.</p></div>';
        return;
    }
    ...
    // Meta Info
    html += '<div style="background: var(--sidebar-bg); border: 1px solid var(--border-color); border-radius: 12px; padding: 20px; margin-bottom: 30px;">';
    html += '<h3 style="margin: 0 0 10px 0; color: var(--accent-color);">🧠 Tổng quan Kiến trúc (Source Intelligence)</h3>';
    html += '<p style="color: var(--text-secondary);">' + data._meta.description + '</p>';
    html += '<div style="font-size: 0.85rem; color: var(--text-muted); margin-top: 10px;">Version: ' + data._meta.version + ' | Cập nhật lần cuối: ' + data._meta.lastUpdated + '</div>';
    html += '</div>';
    
    // Guardrails
    if (data.guardrails && data.guardrails.length > 0) {
        html += '<h3 style="margin-bottom: 15px; color: var(--danger);">🚨 Rào cản Kỹ thuật (Guardrails)</h3>';
        html += '<div class="card" style="margin-bottom: 30px; border-left: 4px solid var(--danger);">';
        html += '<ul style="margin: 0; padding-left: 20px;">';
        data.guardrails.forEach(g => {
            html += '<li style="margin-bottom: 8px;">' + g + '</li>';
        });
        html += '</ul></div>';
    }
    ...
    if (data.databaseSchema) {
        html += '<h3 style="margin-bottom: 15px; color: var(--primary);">🗄️ Database Schema</h3>';
        html += '<div class="grid" style="margin-bottom: 30px;">';
        for (const [name, schema] of Object.entries(data.databaseSchema)) {
            html += '<div class="card">';
            html += '<div class="card-title" style="color: var(--primary); font-family: monospace;">' + name + '</div>';
            html += '<p style="font-size: 0.9rem; color: var(--text-secondary);">' + schema.description + '</p>';
            if (schema.fields) {
                html += '<div style="margin-top: 10px;"><strong>Schema Fields:</strong> <ul style="margin: 5px 0 0 0; padding-left: 20px; font-size: 0.85rem; color: var(--text-muted);">';
                for (const [fName, fType] of Object.entries(schema.fields)) {
                    html += '<li><span style="color: var(--accent-color);">' + fName + '</span>: ' + fType + '</li>';
                }
                html += '</ul></div>';
            }
            html += '</div>';
    ...
    // Collections
    if (data.collections) {
        html += '<h3 style="margin-bottom: 15px; color: var(--success);">📂 Collections Mapping</h3>';
        html += '<div class="grid" style="margin-bottom: 30px;">';
        for (const [name, col] of Object.entries(data.collections)) {
            html += '<div class="card">';
            html += '<div class="card-title" style="color: var(--success); font-family: monospace;">' + name + '</div>';
            html += '<p style="font-size: 0.9rem; color: var(--text-secondary);">' + col.description + '</p>';
            if (col.keyFields) {
                html += '<div style="margin-top: 10px;"><strong>Key Fields:</strong> <span style="font-family: monospace; font-size: 0.85rem; color: var(--accent-color);">' + col.keyFields.join(', ') + '</span></div>';
            }
            if (col.usedIn) {
                html += '<div style="margin-top: 10px;"><strong>Used In:</strong> <ul style="margin: 5px 0 0 0; padding-left: 20px; font-size: 0.85rem; color: var(--text-muted);">';
                col.usedIn.forEach(u => html += '<li>' + u + '</li>');
                html += '</ul></div>';
            }
            html += '</div>';
        }
    ...
    if (data.apiEndpoints) {
        html += '<h3 style="margin-bottom: 15px; color: #f59e0b;">🌐 API Endpoints</h3>';
        html += '<div class="grid" style="margin-bottom: 30px;">';
        for (const [group, endpoints] of Object.entries(data.apiEndpoints)) {
            html += '<div class="card" style="border-left: 3px solid #f59e0b;">';
            html += '<div class="card-title" style="color: #f59e0b;">' + group + '</div>';
            html += '<ul style="margin: 10px 0 0 0; padding-left: 20px; font-size: 0.85rem; color: var(--text-secondary);">';
            for (const [path, desc] of Object.entries(endpoints)) {
                html += '<li style="margin-bottom: 6px;"><code style="color: var(--accent-color);">' + path + '</code><br>' + desc + '</li>';
            }
            html += '</ul>';
            html += '</div>';
        }
    ...
    // Integrations
    if (data.keyIntegrations) {
        html += '<h3 style="margin-bottom: 15px; color: #a855f7;">🔌 Tích hợp Hệ thống</h3>';
        html += '<div class="grid">';
        for (const [name, int] of Object.entries(data.keyIntegrations)) {
            html += '<div class="card" style="border-left: 3px solid #a855f7;">';
            html += '<div class="card-title" style="color: #a855f7;">' + name + '</div>';
            html += '<p style="font-size: 0.9rem; color: var(--text-secondary);">' + int.description + '</p>';
            if (int.files) {
                html += '<div style="margin-top: 10px;"><strong>Files:</strong> <div style="font-family: monospace; font-size: 0.85rem; color: var(--text-muted);">' + int.files.join('<br>') + '</div></div>';
            }
            html += '</div>';
        }
        html += '</div>';
