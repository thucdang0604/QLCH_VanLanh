...
function matchesSlug(svc: any, slug: string): boolean {
...
export default function CategoryPage({ params }: { params: Promise<{ slug: string }> }) {
    ...
    const constraints = useMemo<QueryConstraint[]>(() => {
        ...
        if (isRepair) {
            // Load ALL services — filter client-side by keywords
            c.push(orderBy('createdAt', 'desc'));
        } else if (isNavProductSlug) {
            // New nav slugs: load all active products, filter client-side by condition/category
            c.push(where('status', '==', 'active'));
            if (sortBy === 'price-asc') c.push(orderBy('price_original', 'asc'));
            else if (sortBy === 'price-desc') c.push(orderBy('price_original', 'desc'));
            else c.push(orderBy('createdAt', 'desc'));
        } else {
            // Legacy product slugs: filter by category + status
            const cat = productInfo?.category;
            if (cat) c.push(where('category', '==', cat));
            c.push(where('status', '==', 'active'));
    ...
    }, [slug, isRepair, isNavProductSlug, sortBy]);

    const collectionName = isRepair ? 'services' : 'products';
    const { data: items, loading } = useFirestoreCollection<any>(collectionName, constraints);
    ...
    const filtered = useMemo(() => {
        ...
        if (isRepair) {
            ...
            if (sidebarBrands.length > 0) list = list.filter(p => sidebarBrands.includes(p.brand));
            ...
            if (filterPrice === '0-5') list = list.filter(p => (p.price_promo || p.price_original) < 5_000_000);
            if (filterPrice === '5-10') list = list.filter(p => { const pr = p.price_promo || p.price_original; return pr >= 5_000_000 && pr < 10_000_000; });
            if (filterPrice === '10-20') list = list.filter(p => { const pr = p.price_promo || p.price_original; return pr >= 10_000_000 && pr < 20_000_000; });
            if (filterPrice === '20+') list = list.filter(p => (p.price_promo || p.price_original) >= 20_000_000);
    ...
    const brands = useMemo(() => {
        ...
        let source = items;
        if (navInfo?.condition === 'new') source = items.filter((p: any) => p.condition === 'new');
        else if (navInfo?.condition === 'used') source = items.filter((p: any) => p.condition === 'used' || p.condition === 'like-new');
        const set = new Set(source.map((p: any) => p.brand).filter(Boolean));
    ...
    const categories = useMemo(() => {
        ...
        const set = new Set(source.map((p: any) => p.category).filter(Boolean));
        return Array.from(set).sort() as string[];
    }, [items, navInfo]);
    ...
    const canonicalUrl = `https://qlch-vanlanh.web.app/category/${slug}`;
                                            ...
                                            <Link key={product.id} href={`/product/${product.id}`} className="bg-white rounded-xl shadow-sm overflow-hidden group hover:shadow-lg transition-shadow border border-gray-100">
                                                ...
                                                <div className="p-3">
                                                    ...
                                                    <div className="mt-2">
                                                        ...
                                                        {hasDiscount && <p className="text-xs text-gray-400 line-through">{formatPrice(product.price_original)}</p>}
                                                    </div>
                                                </div>
                                            </Link>
                                        );
