...
export async function POST(request: NextRequest) {
    try {
        ...
        if (name.length < 2 || name.length > 100) {
            return NextResponse.json({ error: 'Vui lòng nhập tên từ 2 đến 100 ký tự.' }, { status: 400 });
        }
        if (phone.length > 20) {
        ...
        if (!content || content.length > 2000) {
            return NextResponse.json({ error: 'Bình luận phải có nội dung và tối đa 2000 ký tự.' }, { status: 400 });
        }
        ...
        console.error('Article comment API error:', error);
